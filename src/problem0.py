...
def problem0a(n):
    ...
    #First you need to get the sums in order to determine if its even or odd, then set up a coniditional statement
...
def problem0b(n):
    ...
    #This is an accumulator pattern problem, immediately put count, for loop, and return count

    count = 0
    for k in range(n-1):
        if is_prime(k+2):                                        # k+2 because normally k would start at 0
...
def problem0c(circle, n, window):
    ...
    #This code below gets the circles on the screen
    #now get the center points and radius- iterative enhancement to simple get one circle

    circle.attach_to(window)
    x_value = circle.center.x + circle.radius*2         #this moves the circles over
    for _ in range(n):                                  # underscore acts the same way as k
        new_circle = rg.Circle(rg.Point(x_value, circle.center.y), circle.radius)
        new_circle.attach_to(window)
        x_value = x_value + circle.radius*2             #This continues increasing the distance
        window.render(0.5)                              #To make animation, pass the window.render a parameter in seconds
